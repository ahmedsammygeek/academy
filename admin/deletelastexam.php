...
if(file_exists("image/$image"))  {
	...
	unlink("image/$image");
	//remove it 
}elseif (file_exists("image/file/$image")) {
	unlink("image/file/$image");
}
else{
	//if this image not exist 
	//go to gallery slider and show alert about this case
	header('location: showlastexam.php?msg=not_exist');
	die();
}

include 'connection.php';
//connection with database(met)
$sql="DELETE FROM last_exams WHERE id='$id' ";
...
if ($num==1) {
	...
	header('location: showlastexam.php?msg=deleted'); die();
}
else
{
	header('location: showlastexam.php?msg=error_delete'); die();
}
 ?>