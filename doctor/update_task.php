...
if(isset($_POST['task_btn'])) {
	...
	foreach ( $inputs as $input_name => $input_value )
	{
		if ( empty( $input_value ) ) {
			header("location: edit_task.php?msg=$input_name");
			die();
		}
	...
	try {
		$task = $conn->prepare("UPDATE tasks SET task_title=?,task_content=?,ex_date=?,subject_id=?,made_by=?,department_id=?,file_exist=?,created_at=?,year=?  ");
		...
		if(isset($_FILES['task_files']['name'][0]) && !empty($_FILES['task_files']['name'][0])) {
			...
			foreach ($uplaoded_files as  $one) {
				$file = $conn->prepare("UPDATE tasks_files SET task_id=?,file=? ");
